...
console.log("html-includes-instrumented.js:start");
...
(scope => {
  ...
  class Includer {
     ...
     */
    loadIncludes(doc) {
      console.log("Includer::loadIncludes()");
     ...
     */
    loadInclude(link) {
      console.log("Includer::loadInclude('" + link.href + "')");
      const url = link.href;
    ...
    makeDocument(resource, url) {
      console.log("Includer::makeDocument()");
      if (!resource) {
    ...
    processIncludesIfLoadingDone() {
      console.log("Includer::processIncludesIfLoadingDone()");
     ...
     */
    runScripts(callback) {
      console.log("Includer::runScripts()");
    ...
    waitForStyles(callback) {
      console.log("Includer::waitForStyles()");
     ...
     */
    fireEvents() {
      console.log("Includer::fireEvents()");
      const n$ = /** @type {!NodeList<!HTMLLinkElement>} */
    ...
    fireEventIfNeeded(link) {
      console.log("Includer::fireEventIfNeeded()");
    ...
    handleMutations(mutations) {
      console.log("Includer::handleMutations(" + muatations + ")");
  ...
  const whenReady = callback => {
    ...
    whenDocumentReady(() => whenIncludesReady(() => callback && callback()));
  }
  ...
  const newCustomEvent = (type, params) => {
    console.log("newCustomEvent(" + type + params + ")");
...
console.log("html-includes-instrumented.js:end");
